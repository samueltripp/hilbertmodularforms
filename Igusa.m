...
if IsIntegral(T[1]) and IsIntegral(T[2]) and IsIntegral(T[3]) and (T[1] ge 0) and (T[2] ge 0) then
...
out := (4*r[1] - r[4]*r[4])*m1*m1 + (4*r[1]+2*r[3]*r[4])*m1 - r[3]*r[3];
return out;
end intrinsic;

intrinsic SiegelMatrix(m1::RngIntElt, r::SeqEnum[FldRatElt]) -> any
{Given m_1, computes m_2 and m}
out := [0/1,0/1,0/1];
out[1] := m1;
out[2] := r[3] - r[4]*m1;
out[3] := r[1] + r[2]*m1;
